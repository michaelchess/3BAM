...
mutation = Table('mutation', post_meta,
    ...
    Column('id', Integer, primary_key=True, nullable=False),
    Column('variant_ID', String(length=200)),
    Column('base_position', String(length=200)),
...
def upgrade(migrate_engine):
    ...
    pre_meta.tables['mutation'].columns['base_pos'].drop()
    post_meta.tables['mutation'].columns['base_position'].create()
...
def downgrade(migrate_engine):
    ...
    pre_meta.bind = migrate_engine
    post_meta.bind = migrate_engine
    pre_meta.tables['mutation'].columns['base_pos'].create()
    post_meta.tables['mutation'].columns['base_position'].drop()
